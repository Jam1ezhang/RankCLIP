...
from rankclip.rank_losses import ListMLE

def get_loss(umodel, outputs, criterion, epoch,options):  
    if(options.inmodal):
    ...
    max_epochs = options.epochs  # Total number of training epochs
    current_epoch = min(epoch, max_epochs)
    rank_weight = (3.0 * (current_epoch - 1)) / (max_epochs - 1)
    rank_weight = min(2.0, max(0.0, rank_weight))  # Clamp between 0 and 2

    rank_score = ListMLE()
    # compute in-modal rank loss
    inmodal_rank_loss = torch.tensor(0).to(options.device)
    if(options.lambda1 > 0):
        logits_per_in_modal_image = umodel.logit_scale.exp() * image_embeds @ image_embeds.t()
        logits_per_in_modal_text = umodel.logit_scale.exp() * text_embeds @ text_embeds.t()
        inmodal_rank_loss = (
            rank_score(logits_per_in_modal_image, logits_per_in_modal_text)
            + rank_score(logits_per_in_modal_text, logits_per_in_modal_image)
        ) / 32
    # compute cross-modal rank loss
    crossmodal_rank_loss = torch.tensor(0).to(options.device)
    if(options.lambda2 > 0):
        crossmodal_rank_loss = (
            rank_score(logits_text_per_image, logits_image_per_text)
            + rank_score(logits_image_per_text, logits_text_per_image)
        ) / 32
        # compute total loss
        rank_loss = (
            rank_weight * (crossmodal_rank_loss / batch_size + inmodal_rank_loss / batch_size)
        )
        loss = contrastive_loss+rank_loss

    
    return loss, contrastive_loss, rank_loss
...
def train(epoch, model, data, optimizer, scheduler, scaler, options):    
    ...
    for index, batch in enumerate(dataloader): 
        ...
        with autocast():
            loss, contrastive_loss, rank_loss = get_loss(umodel, outputs, criterion, epoch, options)
        ...
        if(options.master and (((index + 1) % modulo == 0) or (index == dataloader.num_batches - 1))):
            ...
            metrics = {"loss": loss.item(), "contrastive_loss": contrastive_loss.item(), "rank_loss": rank_loss.item(), "time": end - start, "lr": optimizer.param_groups[0]["lr"]}
            if(options.wandb):
